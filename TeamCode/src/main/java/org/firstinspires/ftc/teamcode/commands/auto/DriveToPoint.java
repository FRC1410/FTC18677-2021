...
public class DriveToPoint extends Command {
    ...
    private PIDHandler angle_PID = new PIDHandler(Constants.ANGLE_P, Constants.ANGLE_I, Constants.ANGLE_D);
    private PIDHandler offset_PID = new PIDHandler(Constants.X_OFFSET_P, Constants.X_OFFSET_I, Constants.X_OFFSET_D);
    private PIDHandler distance_PID = new PIDHandler(Constants.DISTANCE_P, Constants.DISTANCE_I, Constants.DISTANCE_D);
    ...
    public boolean isFinished() {
        if (ending_speed == 0) {
            return (Math.abs(translated_z) < Constants.DISTANCE_FINISHED_THRESHOLD);
        } else {
