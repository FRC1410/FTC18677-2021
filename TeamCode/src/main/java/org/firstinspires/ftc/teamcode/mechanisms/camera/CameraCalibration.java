...
public class CameraCalibration {
	...
	//			Imgproc.circle(in,points[3],20,new Scalar(255,0,255),2);
		...
		if(detectionCount==MIN_STEPS) {
			...
			telemetry.addData("Camera perspective calibrated. Successful detections",""+MIN_STEPS);
			detectionCount++;
		}
		if(detectionCount>=MIN_STEPS) {
			Mat td=new Mat();
		...
		if(isCalibrated()) {
			...
			telemetry.addData("Successful overall calibration. Number of successful ring hue detections",""+MIN_STEPS);
		}
		return false;
	}
